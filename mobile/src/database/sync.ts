...
export async function syncDatabase() {
  await synchronize({
    ...
    pullChanges: async ({ lastPulledAt, schemaVersion, migration }) => {
      ...
      return {
        changes: {
          trips: { created: (tripsData || []).map(mapTrip), updated: [], deleted: [] },
          profiles: { created: (profilesData || []).map(mapProfile), updated: [], deleted: [] },
          chats: { created: (chatsData || []).map(mapChat), updated: [], deleted: [] },
          messages: { created: (messagesData || []).map(mapMessage), updated: [], deleted: [] },
        },
        timestamp: Date.now(),
      };
    },
