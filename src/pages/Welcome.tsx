...
const Welcome = () => {
  ...
  return <div className="min-h-screen bg-gradient-hero flex flex-col safe-top safe-bottom">
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center pt-12 px-[22px] py-[30px]">
        ...
        <div className="relative w-full max-w-sm aspect-[3/4] rounded-3xl overflow-hidden shadow-xl animate-fade-up">
          ...
          <div className="absolute bottom-20 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
            {ladakhImages.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`h-1.5 rounded-full transition-all duration-300 ${index === currentSlide ? "w-6 bg-white" : "w-1.5 bg-white/50"}`} />)}
          </div>
        </div>
      </div>
      ...
      <div className="px-6 pb-8 space-y-6 animate-slide-up">
        ...
        {/* Next Button */}
        <Button onClick={() => navigate("/auth")} className="w-full h-14 text-lg font-semibold rounded-2xl shadow-glow">
          Next
        </Button>
      </div>
    </div>;
